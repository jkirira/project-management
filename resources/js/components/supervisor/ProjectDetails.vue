        ...
        <div class="container px-5 py-24 mx-auto">
            <div class="flex items-center border-b pb-10 border-gray-200 flex-col lg:flex-row">
                ...
                <div class="flex-grow sm:text-left text-center mt-6 sm:mt-0">
                    ...
                    <h2>
                        Manager: {{ (project.manager_details) ? project.manager_details.user.first_name + ' ' + project.manager_details.user.last_name : 'none' }}
                    </h2>
                    <h2>Units: {{ project.units_count }}</h2>
                </div>
            </div>

            <template v-if="userDetails.role_id == 3">
                    ...
                    <div class="p-2 w-3/4 my-5">
                        ...
                        <div class="relative">
                            <select id="manager_id" v-model="form.manager_id" name="manager_id" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                ...
                                <option v-for="(manager, index) in managers" :key="index" :value="manager.id">{{ manager.first_name + " " + manager.last_name }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="p-2 w-full my-6">
                        <button @click="assignManager" :disabled="button_disabled" class="flex text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">
                            {{ button_text }}
                        </button>
                    </div>
                </div>
    ...
    methods: {
        validate() {
            if (!this.form.manager_id || this.form.manager_id == '') {
                this.$swal("Error: Please pick a manager")
                return false;
            }
            return true;
        },
        ...
        assignManager() {
            ...
            axios({
                method: 'post',
                url: base_url + '/api/managers/project/' + this.$router.params.id,
                data: this.form,
                headers: {
                    Authorization: 'Bearer ' + window.localStorage.getItem('token')
                }
                ...
                console.log(err)
                this.$swal("Error", "could not assign this project.")
            }).then(() => {
                this.button_disabled = false
                this.button_text = 'Assign'
            })
        },
